...
source("loadData.R")
ls()
rm(FINAL_DATA, passive_n_phq2, mdata)
phq2 <- phq2 %>% filter(week <= 12) %>% filter(!is.na(sum_phq2))
phq9 <- phq9 %>% filter(week <= 12) %>% filter(!is.na(sum_phq9))
passive_data <- passive_data %>% filter(week <= 12)
```
...
mobilityfeatures <- passive_data %>% 
  select(user_id, day, week, mobility, mobility_radius, passive_date_pacific) %>%
...
tmp_metaData <- metaData %>% select(Gender, Age, user_id ) %>% 
  filter(!is.na(user_id),
         !is.na(Gender),
         Gender != '')
mobilityfeatures <- merge(tmp_metaData, mobilityfeatures, by="user_id")
mobilityfeatures <- mobilityfeatures %>% filter(! (is.na(Age) | Age == '')) %>%
  filter(Age >=18)
n_distinct(mobilityfeatures$user_id)
```
...
mobilityfeatures <- mobilityfeatures %>% dplyr::mutate(user_id = as.character(user_id))
...
### After cleaning number of individuals for whom we have mobility data
```{r}
n_distinct(mobilityfeatures_n_phq2$user_id)
```


### Filter to users with enough data
```{r}
dataPerUser <- mobilityfeatures_n_phq2 %>% group_by(user_id) %>% 
  summarise(n = length(unique(day)),
            survey = 'passive')

hist(dataPerUser$n)
#select individuals with atleast 28 data points (AKA 4 weeks)
SELECTED_USERS <- dataPerUser %>% filter( n >= 28) %>% .$user_id %>% unique()
mobilityfeatures_n_phq2_flt <- mobilityfeatures_n_phq2 %>% filter(user_id %in% SELECTED_USERS)
```

```{r}
n_distinct(mobilityfeatures_n_phq2_flt$user_id)
```
...
```{r}
tmp_selected_users <- unique(mobilityfeatures_n_phq2_flt$user_id)

cor_mobRadiusTrans <- ldply(tmp_selected_users, function(userId){
  n1 <- mobilityfeatures_n_phq2_flt %>% filter(user_id == userId)
...
sum(cor_mobRadiusTrans$pval.adj < .05, na.rm = T)


tmp <- cor_mobRadiusTrans %>% filter(p.value < .05) %>% arrange(p.value, desc(abs(cor)))
View(tmp)
...
ggsave("mobilityAnalysis/plots/mobility.png", pl, height=5, width=7, units="in")
ggsave("mobilityAnalysis/plots/mobility.tiff", pl, height=5, width=7, units="in")
```
...
tmp_data <- mobilityfeatures_n_phq2_flt %>% select( -day,-week, -Age, -start) 
colnames(tmp_data)
...
tmp_phq9 <-  as.data.frame(phq9) %>% select(user_id, sum_phq9, week)
...
selected_users <- tmp_mobF %>% group_by(user_id) %>% summarise(numDataPoints = n()) %>% 
  filter(numDataPoints >= 15) %>% .$user_id
tmp_mobF <- tmp_mobF %>% filter(user_id %in% selected_users)

cor_mobRadius_phq9 <- ldply(selected_users, function(userId){
  n1 <- tmp_mobF %>% filter(user_id == userId)
  tmp_cor <- cor(n1$mobRadius_trans, n1$sum_phq9, method="spearman")
  tmp_pval <- cor.test(n1$mobRadius_trans, n1$sum_phq9, method="spearman")['p.value']
  data.frame(cor=tmp_cor, pval=tmp_pval, user_id = userId)
})

View(cor_mobRadius_phq9)
sum(cor_mobRadius_phq9$p.value < .05, na.rm = T)
cor_mobRadius_phq9['pval.adj'] <- p.adjust(cor_mobRadius_phq9$p.value, method="fdr")
sum(cor_mobRadius_phq9$pval.adj < .05, na.rm = T)
...
phq2_correlation_by_age <- mobilityfeatures_n_phq2_flt %>% group_by(Age) %>% summarise(cor = cor(sum_phq2, mobRadius_trans, method="spearman"))
...
#Mobility Radius
x <- mobilityfeatures_n_phq2_flt %>% group_by(user_id, week) %>%
...
x1$user_id <- NULL
rowAnnot <- metaData %>% filter(user_id %in% rownames(x1)) %>%
  select(Age,Gender, user_id)
